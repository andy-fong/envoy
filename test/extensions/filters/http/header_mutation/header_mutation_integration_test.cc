...
public:
  ...
  // if "disabled" is false, that means we are testing enabling the filter at various level, so the filter is set up
  // to be disabled by default. Then, enable at the specified "route_level"
  // If "disabled" is true, that means we are testing disabling the filter at various level, so the filter is set up
  // to be enabled by default. Then, disable at the specified "route_level"
  void initializeUpstreamFilterForSpecificityEnableDisableTest(bool most_specific_header_mutations_wins,
                                        RouteLevelFlag route_level, bool disabled = false) {
    setUpstreamProtocol(FakeHttpConnection::Type::HTTP1);
    envoy::extensions::filters::http::header_mutation::v3::HeaderMutation header_mutation;
    TestUtility::loadFromYaml(upstream_header_mutation_config_, header_mutation);
    envoy::extensions::filters::network::http_connection_manager::v3::HttpFilter
...
//  initializeUpstreamFilterForSpecificityDisableTest(true, VirtualHostLevel);
